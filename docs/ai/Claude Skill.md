...
### 📂 第一步：建立"技能库"
Claude Code 会自动扫描用户主目录下的 `.claude/skills` 文件夹。

**标准路径结构**（请务必遵守此层级）：
```text
~/.claude/skills/                      <-- skill所在目录（~代表用户主目录）
│
...
**Mac 路径说明**：
- `~` 符号代表你的用户主目录（例如 `/Users/zhengzhuang/`）
- 完整路径示例：`/Users/alex/.claude/skills/`
- 可以在终端中用 `echo ~/.claude/skills` 查看完整路径
...
**文件内容示例 (`~/.claude/skills/xlsx/SKILL.md`)**：
````markdown
...
### Mac 特定提示
- **权限管理**：Mac 可能会首次运行时弹出权限提示，需要允许终端访问文件系统
- **Python 环境**：建议使用 Homebrew 安装 Python：`brew install python@3.11`
- **路径大小写**：Mac 文件系统默认不区分大小写（APFS），但建议保持文件夹名小写以兼容 Linux

## 4. 最终文件夹层级结构 - 样例：

```text
~/.claude/skills/                              <-- 【根目录】所有 Skill 的存放位置
│
...
```bash
# 创建技能目录结构
mkdir -p ~/.claude/skills/pptx-creation/{scripts,assets}
mkdir -p ~/.claude/skills/xlsx-analysis/{scripts,examples}

# 验证目录创建成功
tree ~/.claude/skills/  # 需要先安装: brew install tree
```